...
export async function AdminDashboardPage({ initialSection }: { initialSection?: AdminSection }) {
  ...
  const integrationsStatus = {
    ...
    emailProvider: process.env.BREVO_API_KEY
      ? "brevo"
      : process.env.GMAIL_AUTH_EMAIL && process.env.GMAIL_APP_PASSWORD
        ? "gmail"
        : null,
    brevoWebhookConfigured: Boolean(process.env.BREVO_WEBHOOK_TOKEN),
    twilioConfigured: Boolean(
      process.env.TWILIO_ACCOUNT_SID &&
        process.env.TWILIO_AUTH_TOKEN &&
        (process.env.TWILIO_FROM_NUMBER || process.env.TWILIO_FROM_NUMBER_AZ || process.env.TWILIO_FROM_NUMBER_FL),
    ),
  };
